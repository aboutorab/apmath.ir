document.addEventListener('DOMContentLoaded',async()=>{
    ...
    await MenuBuilder.buildToolbarScripts();
    ...
    const open=()=>{
        ...
        if(window.innerWidth<=768){
            if(sH)sH.style.display='none';if(tM)tM.style.display='none';
    ...
    const close=()=>{
        ...
        if(window.innerWidth<=768){if(sH)sH.style.display='';if(tM)tM.style.display='';if(mMM)mMM.style.display=''}
    };
    ...
    document.addEventListener('click',async e=>{
        const pl=e.target.closest('[data-post]');
        if(pl){e.preventDefault();const s=pl.dataset.post;const jsn=pl.dataset.jsn||'';if(s){R.navigate(`?post=${encodeURIComponent(s)}${jsn?`&jsn=${encodeURIComponent(jsn)}`:''}`);window.scrollTo({top:0,behavior:'smooth'});if(window.innerWidth<=768)close()}}
    });
