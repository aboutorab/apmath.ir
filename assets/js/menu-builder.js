class MenuBuilder {
    static async buildMainMenu() {
        const ml = document.getElementById('mainMenuList');
        if (!ml) return;
        try {
            const r = await fetch(BlogEngine.acb('data/menu.json'));
            if (!r.ok) { ml.innerHTML = '<li><span class="loading-text">منو در دسترس نیست</span></li>'; return; }
            const items = await r.json();
            if (!items?.length) { ml.innerHTML = ''; return; }
            ml.innerHTML = this.buildMenuTree(items, 0);
        } catch (e) { ml.innerHTML = '<li><span class="loading-text">خطا در بارگذاری</span></li>'; }
    }

    static async buildMobileMenu() {
        const mml = document.getElementById('mobileMenuList');
        if (!mml) return;
        try {
            const r = await fetch(BlogEngine.acb('data/menu.json'));
            if (!r.ok) { mml.innerHTML = '<li><span class="loading-text">منو در دسترس نیست</span></li>'; return; }
            const items = await r.json();
            if (!items?.length) { mml.innerHTML = ''; return; }
            mml.innerHTML = this.buildMenuTree(items, 0);
            this.attachMobileMenuEvents();
        } catch (e) { mml.innerHTML = '<li><span class="loading-text">خطا در بارگذاری</span></li>'; }
    }
    ...
    static buildMenuTree(items, level = 0) {
        ...
        items.forEach(item => {
            ...
            if (hc && level < 3) {
                ...
                h += this.buildMenuTree(item.children, level + 1);
                h += `</ul></li>`;
            } else {
                h += `<li><a href="${item.link || '#'}"${ta}>${item.title}</a></li>`;
            }
        });
        return h;
    }
    ...
    static attachMobileMenuEvents() {
        ...
        mm.addEventListener('click', function(e) {
            const tl = e.target.closest('a[data-menu-parent]');
            if (!tl) return;
            e.preventDefault(); e.stopPropagation();
            const pl = tl.parentElement;
            const sm = pl.querySelector(':scope > .submenu');
            if (!sm) return;
            ...
            if (sm.classList.contains('mobile-open')) {
                ...
                sm.style.display = 'flex'; sm.classList.add('mobile-open'); tl.classList.add('open');
            }
        });
        mm.addEventListener('click', function(e) {
    ...
    static setupMobileMenuToggle() {
        ...
        tb.addEventListener('click', function() {
            if (mcm.classList.contains('open')) { mcm.classList.remove('open'); mcm.style.maxHeight = '0'; tb.classList.remove('open'); }
            else { mcm.classList.add('open'); mcm.style.maxHeight = mcm.scrollHeight + 'px'; tb.classList.add('open'); }
        });
        document.addEventListener('click', function(e) {
            if (!e.target.closest('#mobileMainMenu') && !e.target.closest('#mobileMenuToggle')) {
                mcm.classList.remove('open'); mcm.style.maxHeight = '0'; tb.classList.remove('open');
            }
        });
    }

    static async buildTopMenu() {
        const ml = document.getElementById('topMenuList');
        if (!ml) return;
        try {
            const r = await fetch(BlogEngine.acb('data/topmenu.json'));
            if (!r.ok) { ml.innerHTML = '<li><span class="loading-text">منو در دسترس نیست</span></li>'; return; }
            const items = await r.json();
            if (!items?.length) { ml.innerHTML = ''; return; }
            let h = '';
            items.forEach(item => { if (item.title && item.link) { const ta = item.target === '_blank' ? ' target="_blank" rel="noopener noreferrer"' : ''; h += `<li><a href="${item.link}"${ta}>${item.title}</a></li>`; } });
            ml.innerHTML = h || '<li><span class="loading-text">بدون لینک</span></li>';
        } catch (e) { ml.innerHTML = '<li><span class="loading-text">خطا در بارگذاری</span></li>'; }
    }
